...
class PincodeService {
  ...
  static Future<Map<String, dynamic>> getAreasByPincode(
    String pincode,
  ) async {
    try {
      // Validate pincode format
      ...
      final url = Uri.parse('${ApiConfig.baseUrl}/pincode/$pincode/areas');
      final response = await http.get(url).timeout(const Duration(seconds: 10));
      ...
      if (response.statusCode == 200 && data['success'] == true) {
        ...
        return {
          'success': false,
          'message': data['message'] ?? 'Areas not found',
        };
      }
    } catch (e) {
      return {'success': false, 'message': 'Failed to fetch areas: $e'};
    }
  }
