...
class _AccountListScreenState extends State<AccountListScreen> {
  ...
  /// Salesmen only (for allotment dropdown / filters)
  ...
  /// Telecallers only (for telecaller filter)
  List<Map<String, dynamic>> get _telecallers {
    return _salesmen.where((u) {
      final role = (u['role'] ?? u['roleId'] ?? '').toString().toLowerCase();
  ...
  Future<Map<String, dynamic>> _fetchAccountsInBackground() async {
    return await AccountService.fetchAccounts(
      ...
      // Telecaller filter: accounts created by selected telecaller
      createdById: _filterTelecallerId,
      // Salesman filter: accounts assigned to selected salesman
      assignedToId: _filterSalesmanId,
  ...
  void _showFilterDialog() {
    showDialog(
      ...
      builder: (context) {
        ...
        String? tmpTelecallerId = _filterTelecallerId;
        String? tmpPincode = _filterPincode;
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        onChanged: (value) =>
                            setDialogState(() => tmpTelecallerId = value),
                      ),
                      const SizedBox(height: 15),
                      TextFormField(
                        initialValue: tmpPincode,
                        decoration: const InputDecoration(
                          labelText: 'Pincode',
                          hintText: 'e.g. 411001',
                        ),
                        ...
                        maxLength: 6,
                        onChanged: (value) =>
                            setDialogState(() => tmpPincode = value.isEmpty ? null : value),
                      ),
                ...
                ElevatedButton(
                  onPressed: () {
                    setState(() {
                      ...
                      _filterTelecallerId = tmpTelecallerId;
                      _filterPincode = tmpPincode?.trim().isEmpty ?? true ? null : tmpPincode?.trim();
  ...
  bool _hasActiveFilters() {
    return _filterCustomerStage != null ||
        ...
        _filterSalesmanId != null ||
        _filterTelecallerId != null ||
        (_filterPincode != null && _filterPincode!.trim().isNotEmpty) ||
        _filterDatePreset != null;
  }
  ...
  String _getActiveFiltersText() {
    ...
    if (_filterTelecallerId != null) {
      final user = _salesmen.firstWhere(
        (s) => (s['id'] ?? s['_id']) == _filterTelecallerId,
        orElse: () => {'name': 'Unknown Telecaller'},
      );
      activeFilters.add('Telecaller: ${user['name'] ?? 'Unknown'}');
    }
    if (_filterPincode != null && _filterPincode!.trim().isNotEmpty) {
      activeFilters.add('Pincode: ${_filterPincode!.trim()}');
    }
